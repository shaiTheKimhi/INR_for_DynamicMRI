...
from inr.utils import fftnc, ifftnc, coil_combine, path_checker, visual_mag, visual_err_mag, gen_traj, NUFFT, metrics_extended, aggregate_dataset_metrics
...
log_path = './log_cmr_spatial/spoke{}_{}'.format(spoke_num, str(datetime.datetime.now().strftime('%y%m%d_%H%M%S')))
...
for patient_idx in range(len(cds)):
    ...
    for z_index in range(nz):
        ...
        # Calculate new smap on the lower resolution image
        ...
        scale_factor_full = torch.abs(img_gt_full).max()
        img_gt_full /= scale_factor_full
        ...
        for e in epoch_loop:
            ...
            if (e + 1) % summary_epoch == 0:
                ...
                if is_example_slice:
                    ...
                    visual_err_mag(intensity, img_gt_full, log_path + '/proposed_{}_{}_abs_err_{}_{}.png'.format(spoke_num, frames, e+1, tag))
        ...
        if best_intensity is not None:
            ...
            metrics_dict = metrics_extended(
                best_intensity, img_gt_full, time_usage, metrics_filename if is_example_slice else None,
            )
            ...
            if is_example_slice:
                ...
                visual_err_mag(best_intensity, img_gt_full, os.path.join(log_path, f'proposed_{tag}_{spoke_num}_{frames}_abs_err_{best_epoch}.png'))

            all_dataset_metrics.append(metrics_dict)
        else:
            print(f"  Warning: {tag} finished without running inference/evaluation.")

        del inr, pos, pos_dense_s, kdata, nufft_op, img, smap, img_gt, img_gt_full, img_full, smap_full, img_fft
        if best_intensity is not None:
            del best_intensity
